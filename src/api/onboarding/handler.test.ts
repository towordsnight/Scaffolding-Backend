...
describe('POST /api/onboarding/declaration', () => {
  ...
  it('returns 400 for invalid stress_baseline', async () => {
    const res = makeRes();
    await declaration(makeReq({ adhd_flag: false, stress_baseline: 5, course_level: 'intro' }), res);
    expect(res.status).toHaveBeenCalledWith(400);
  });
  ...
  it('returns 403 when consent not given', async () => {
    ...
    const res = makeRes();
    await declaration(makeReq({ adhd_flag: false, stress_baseline: 0, course_level: 'intro' }), res);
  ...
  it('returns 200 and writes thresholds on happy path', async () => {
    ...
    const res = makeRes();
    await declaration(makeReq({ adhd_flag: false, stress_baseline: 1, course_level: 'intermediate' }), res);
